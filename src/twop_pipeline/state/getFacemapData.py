...
def get_state_df(facemap_data, camera_times, 
    ...
    if cam_fps is None:
        cam_fps = get_camera_fps(camera_times, method='total')
    ...
    def get_rescaled_signal(filtered_signal, movement_percentile, signal_name=None):
        ...
        # OLD (caused drift via synthetic sampling rate):
        ...
        # NEW (correct): interpolate directly onto camera_times
        signal_times = np.linspace(camera_times[0], camera_times[-1], len(filtered_signal))
        stretched_signal_smooth = np.interp(camera_times, signal_times, filtered_signal)

        # Boolean interpolation (nearest-like)
        stretched_bool = np.interp(camera_times, signal_times, bool_chunk.astype(float)) > 0.5

        return stretched_signal_smooth, stretched_bool
    
    facial_motion_stretched, motion_bool = get_rescaled_signal(
        motion_smoothed, movement_percentile,
    ...
    if treadmill_signal is not None:
        ...
        treadmill_stretched, locomotion_bool = get_rescaled_signal(
            treadmill_smoothed, movement_percentile,
            signal_name='treadmill'
        )

        facial_motion_df.insert(loc=2, column='locomotion_bool', value=locomotion_bool)  
        facial_motion_df['treadmill'] = treadmill_stretched

    if 'pupil' in facemap_data and pupil_data:
        pupil_area = facemap_data['pupil'][0]['area_smooth']
        pupil_smoothed = gaussian_filter1d(pupil_area, sigma=smoothing_kernel)

        pupil_stretched, pupil_bool = get_rescaled_signal(
            pupil_smoothed, movement_percentile,
            signal_name='pupil'
        )

        facial_motion_df.insert(loc=3, column='pupil_bool', value=pupil_bool)  
        facial_motion_df['pupil_area'] = stats.min_max_norm(pupil_stretched) if min_max_norm else pupil_stretched
    ...
    if paw_data:
        ...
        if 'left_paw' in motion_indices.keys():
            ...
            left_paw_stretched, left_paw_bool = get_rescaled_signal(
                left_paw_smoothed, movement_percentile,
                signal_name='left paw'
            )

            facial_motion_df['left_paw'] = stats.min_max_norm(left_paw_stretched) if min_max_norm else left_paw_stretched
        ...
        if 'right_paw' in motion_indices.keys():
            ...
            right_paw_stretched, right_paw_bool  = get_rescaled_signal(
                right_paw_smoothed, movement_percentile,
                signal_name='right paw'
            )

            facial_motion_df['right_paw'] = stats.min_max_norm(right_paw_stretched) if min_max_norm else right_paw_stretched
